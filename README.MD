# PACT CONTRACT TEST Demo

这是一个提供了使用PACT框架来进行Contract Test的用例项目，在消费者（Consumer）使用了Junit，
并且在服务提供方（Provider）中使用了Gradle task。项目包含：
...
- [PACT Test Demo](#pact-contract-test-demo)
  * [1. Clone项目到你的本地](#1-clone)
    + [1.1 Test Provider](#11-test-provider)
    + [1.2 Demo Consumer Burger](#12-demo-consumer-burger)
...
### 1.1. Test Provider
This is an API backend service which serves at http://localhost:8080/cart, consumers
can retrieve shopping cart's product information by calling this endpoint with a query parameter **prdname**,
to start the provider:

`./gradlew :test-provider:bootRun`

then call http://localhost:8080/cart?prdname=Burger will get:

![](https://github.com/Mikuu/Pact-JVM-Example/blob/master/screenshot/provider.miku.png)

and call http://localhost:8080/cart?prdname=Chicken will get:

![](https://github.com/Mikuu/Pact-JVM-Example/blob/master/screenshot/provider.nanoha.png)

### 1.2. Demo Consumer Burger
This is the first example consumer we called Burger, to start it:

`./gradlew :demo-consumer-burger:bootRun`

then visit http://localhost:8081/burger in your browser, you can get this:
...
### 1.3. Demo Consumer Chicken
This is the second example consumer we called Chicken, to start it:

`./gradlew :demo-consumer-chicken:bootRun`

then visit http://localhost:8082/chicken in your browser, you can get this:
